...
static BLEUUID ledCharacteristicUUID("01234567-0123-4567-89ab-0123456789fg");
...
//When the BLE Server sends a new led reading with the notify property
...
void loop() {
  ...
  if (doConnect == true) {
    if (connectToServer(*pServerAddress)) {
      ...
      // ledCharacteristic->getDescriptor(BLEUUID((uint16_t)0x2904))->writeValue((uint8_t*)notificationOn, 2, true); // Led 
      connected = true;
    } else {
  ...
  if(digitalRead(BUTTON_B_PIN) == LOW){
    Serial.println("Btn B pressed! Turning off LED...");
  
    // // Prepare the "off" command (based on server, LED off is '1')
    // const char ledOffCommand[] = "1";
    
    // Send the "off" command to the LED characteristic
    ledCharacteristic->writeValue("abc");
